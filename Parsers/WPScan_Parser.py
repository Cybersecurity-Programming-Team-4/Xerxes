...
class WPScan_Parser:
    ...
    def Parse_WPScan(self):
        ...
        file_name = "../Test_Documents/" + self.IP + "_WPScan.txt"
        print(file_name)
        try:
            infile = open(file_name, 'r')
        except:
            logging.debug("Specified WordPress Scan file for {} not found".format(self.IP))
        else:
            ...
            for line in infile:
                ...
                if "WordPress version" in line:
                    ...
                    if version_line[3] != "can":
                        Xerxes_SQL.update_site_entry(db, self.IP, "CMS", "WordPress " + version_line[3])
                if "WordPress theme in use: " in line:
                    theme_line = line.rsplit()
                    Xerxes_SQL.insert_into_site_vulnerabilities(db, self.IP, "Theme",
                    ...
                    for line in infile:
                        if "Name:" in line:
                            ...
                            if Xerxes_SQL.CMS_extension_lookup(Xerxes_SQL.connect_database(), plugin_line[2], "WordPress"):
                                Xerxes_SQL.insert_into_site_vulnerabilities(db, self.IP, "Plugin", inserted_line)
            db.close()
            os.remove(file_name)
