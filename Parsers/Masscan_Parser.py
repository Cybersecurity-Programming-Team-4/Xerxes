        ...
        for port in portsList:
            if port.attributes['portid'].value != 0:    # Disregard host check scan
                ...
                if len(serviceList) == 0:   # For blank services
                    Xerxes_SQL.insert_into_host_open_services(db, address, port.attributes['portid'].value, "None", "None")
                else:
                    for service in serviceList:
                        Xerxes_SQL.insert_into_host_open_services(db, address, port.attributes['portid'].value, service.attributes['name'].value, service.attributes['banner'].value)

        # Add with just initial info
        Xerxes_SQL.insert_into_whois(db, address, WHOIs_Response)
        Xerxes_SQL.insert_host_entry(db, address, returnedName, addressType, region, timeStr)
    db.close()
